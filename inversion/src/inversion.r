...
#format truth vector to average over domain cells
struth_mat <- matrix(struth, nrow = ntimes, byrow = T)
struth_avgs <- apply(struth_mat, FUN = function(x) W %*% x, MARGIN = 1) #weight by cell areas
struth_avg <- mean(struth_avgs)
...
if(shat_avg < .01){
  ...
  struth_avg_char <- formatC(struth_avg, format = "e", digits = 3)
} else{
	shat_avg_char <- round(shat_avg, 3)
	sprior_avg_char <- round(sprior_avg, 3)
  struth_avg_char <- round(struth_avg, 3)
}


print(paste("Overall domain-averaged prior emissions:", sprior_avg_char, flux_units))
print(paste("Overall domain-averaged true emissions:", struth_avg_char, flux_units))
print("#############")
...
struth_subset_avg <- mean(struth_avgs[isubset])
...
if(!is.na(shat_subset_avg) &  shat_subset_avg < .01){
  ...
  struth_avg_char <- formatC(struth_subset_avg, format = "e", digits = 3)
} else{
	shat_avg_char <- round(shat_subset_avg, 3)
	sprior_avg_char <- round(sprior_subset_avg, 3)
  struth_avg_char <- round(struth_subset_avg, 3)
}
...
if ((flux_t_res < 24*3600) & (subset_hour_begin != 0 | subset_hour_end != 23)) {
    ...
    print(paste(subset_hour_begin, "-", subset_hour_end, "UTC average of true emissions:",
        struth_avg_char, flux_units))
